...
export async function revokeMintAfter({
  mint,
  userWallet,
}: RevokeAfterParams) {
  ...
  try {
    ...
    if (mintInfo.mintAuthority === null) {
      throw new Error(
        `Mint authority is already revoked for mint ${mint.toBase58()}. Verify on Solana Explorer: https://explorer.solana.com/address/${mint.toBase58()}?cluster=devnet`
      );
    }
    ...
    throw new Error(
      error.message || `Invalid mint address or issue with mint authority: ${mint.toBase58()}`
    );
  }
