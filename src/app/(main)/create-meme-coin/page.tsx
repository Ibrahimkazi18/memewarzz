...
export default function TokenManagementPage() {
  const [formData, setFormData] = useState({
    ...
    revokeFreeze: true, // This is disabled and always true in the original code
  })
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-12 space-y-10">
      ...
      <section className="text-center space-y-3">
        <h1 className="text-4xl font-bold tracking-tight sm:text-5xl">Meme Coin Tools</h1>
        <p className="text-lg text-muted-foreground">Manage your meme tokens and liquidity pools with ease.</p>
      </section>

      <Tabs defaultValue="create-token" className="w-full">
        <TabsList className="grid w-full grid-cols-2 sm:grid-cols-4 h-12">
          <TabsTrigger value="create-token" className="text-base flex items-center gap-2">
            <Coins className="w-4 h-4" /> Create Token
          </TabsTrigger>
          <TabsTrigger value="burn-coin" className="text-base flex items-center gap-2">
            <Flame className="w-4 h-4" /> Burn Coin
          </TabsTrigger>
          <TabsTrigger value="add-liquidity" className="text-base flex items-center gap-2">
            <Plus className="w-4 h-4" /> Add Liquidity
          </TabsTrigger>
          <TabsTrigger value="remove-liquidity" className="text-base flex items-center gap-2">
        ...
        <TabsContent value="create-token" className="mt-8 space-y-8">
          <section className="bg-muted p-6 rounded-xl shadow-lg space-y-4">
            ...
            <div className="bg-card text-card-foreground p-3 rounded-lg text-center text-sm font-medium border border-primary/20">
              Total cost: <span className="font-semibold text-primary">0.3 SOL</span> – includes all creation fees.
            </div>
          </section>
          ...
          <section>
            <Card className="shadow-lg">
              <CardHeader className="text-center">
                ...
                <CardDescription>Fill in the details to create your new token.</CardDescription>
              </CardHeader>
              <CardContent className="space-y-6">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div className="space-y-2">
                    <Label htmlFor="token-name">Token Name (Max 32 Characters)</Label>
                    <Input
                      id="token-name"
                  ...
                  <div className="space-y-2">
                    <Label htmlFor="token-symbol">Token Symbol (Max 8 Characters)</Label>
                    <Input
                      id="token-symbol"
                      placeholder="Eg: MEME"
                      maxLength={8}
                  ...
                  <div className="space-y-2">
                    ...
                    <Input
                      ...
                      min={1}
                      max={9}
                      onChange={(e) => setFormData({ ...formData, decimals: Number.parseInt(e.target.value || "0") })}
                    />
                    {errors.decimals && <p className="text-destructive text-sm mt-1">{errors.decimals}</p>}
                  </div>
                  <div className="space-y-2">
                    ...
                    />
                    {errors.totalSupply && <p className="text-destructive text-sm mt-1">{errors.totalSupply}</p>}
                  </div>
                </div>
                ...
                <div className="space-y-2">
                  ...
                  <div className="relative flex flex-col items-center justify-center p-6 border-2 border-dashed rounded-lg cursor-pointer bg-muted hover:bg-muted/80 transition-colors">
                    ...
                    <span className="text-sm font-medium text-muted-foreground">
                      {formData.file?.name || "Click to upload or drag and drop"}
                    </span>
                    <input
                      id="file-upload"
                      type="file"
                ...
                <div className="flex items-center justify-between p-4 border rounded-lg bg-card">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-sm text-muted-foreground">(0.1 SOL)</span>
                  </div>
                </div>
          ...
          <section className="space-y-4">
            <Card className="shadow-lg">
              <CardHeader className="text-center">
                <CardTitle className="text-2xl">Revoke Mint Authority (Deployed Tokens)</CardTitle>
                <CardDescription>For tokens already created without mint authority revoked.</CardDescription>
              </CardHeader>
        ...
        <TabsContent value="burn-coin" className="mt-8">
          <Card className="shadow-lg">
            <CardHeader className="text-center">
              ...
              <CardDescription>Reduce the total supply of your token by burning a specified amount.</CardDescription>
            </CardHeader>
            <CardContent className="space-y-6">
              <div className="space-y-2">
                <Label htmlFor="token-address-burn">Token Address</Label>
                <Input id="token-address-burn" placeholder="Enter token address (e.g., 7x...)" />
              </div>
              <div className="space-y-2">
                <Label htmlFor="amount-to-burn">Amount to Burn</Label>
                <Input id="amount-to-burn" type="number" placeholder="e.g., 1000000" min="0" />
              </div>
        ...
        <TabsContent value="add-liquidity" className="mt-8">
          <Card className="shadow-lg">
            <CardHeader className="text-center">
              ...
              <CardDescription>Create or add liquidity to a trading pair for your token.</CardDescription>
            </CardHeader>
            <CardContent className="space-y-6">
              <div className="space-y-2">
                <Label htmlFor="token-address-lp">Your Token Address</Label>
                <Input id="token-address-lp" placeholder="Enter your token address" />
              </div>
              <div className="space-y-2">
                <Label htmlFor="sol-amount">SOL Amount</Label>
                <Input id="sol-amount" type="number" placeholder="e.g., 10" min="0" />
              </div>
              <div className="space-y-2">
                <Label htmlFor="token-amount">Your Token Amount</Label>
                <Input id="token-amount" type="number" placeholder="e.g., 10000000" min="0" />
              </div>
              <Button className="w-full" size="lg">
                Add Liquidity
              </Button>
        ...
        <TabsContent value="remove-liquidity" className="mt-8">
          <Card className="shadow-lg">
            <CardHeader className="text-center">
              ...
              <CardDescription>Withdraw your assets from an existing liquidity pool.</CardDescription>
            </CardHeader>
            <CardContent className="space-y-6">
              <div className="space-y-2">
                <Label htmlFor="lp-token-address">LP Token Address</Label>
                <Input id="lp-token-address" placeholder="Enter LP token address" />
              </div>
              <div className="space-y-2">
                <Label htmlFor="lp-amount">Amount of LP Tokens to Remove</Label>
                <Input id="lp-amount" type="number" placeholder="e.g., 50" min="0" />
              </div>
              <Button className="w-full" size="lg">
                Remove Liquidity
              </Button>
